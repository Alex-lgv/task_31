...
export class DragAndDrop {
  constructor(document) {
    ...
    this.dropF = function (e, status) {
      e.preventDefault();
      const idUser = Task.getUserId(e.dataTransfer.getData('text/plain'));
      if (idUser != -1) {
        Task.changeStatus(e.dataTransfer.getData('text/plain'), status);
        this.removedListenerAll();
        startAll(this.document);
      }
    };
    ...
    this.elB.forEach((element) => {
      element.addEventListener('dragstart', (e) => this.start(e));
    });
    this.elR.forEach((element) => {
      element.addEventListener('dragstart', (e) => this.start(e));
    });
    this.elP.forEach((element) => {
      element.addEventListener('dragstart', (e) => this.start(e));
    });
    this.elF.forEach((element) => {
      element.addEventListener('dragstart', (e) => this.start(e));
    });
  }
  ...
  start(ev) {
    ...
    const idUser = Task.getUserId(ev.target.id);
    const statusTask = Task.getUserStatus(ev.target.id);
  ...
  removedListener(nameClass) {
    this.document
      ...
      .removeEventListener('dragover', this.dragOverF);
    this.document
      .querySelector(nameClass)
      .removeEventListener('drop', this.dropF);
  }
  ...
  addListenerChild(nameClass, status) {
    this.document
      .getElementById(nameClass)
      .addEventListener('dragover', this.dragOverF);
    this.document
      .getElementById(nameClass)
      .addEventListener('drop', (e) => this.dropF(e, status));
  }
}
